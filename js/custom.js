...
jQuery(document).ready(function ($) {
	...
	if ($("#contactForm-widget1")[0]) {
		$('#contactForm-widget1').submit(function () {
			$('#contactForm-widget1 .error').remove();
			$('#contactForm-widget1 .requiredField').removeClass('fielderror');
			$('#contactForm-widget1 .requiredField').addClass('fieldtrue');
			$('#contactForm-widget1 span strong').remove();
			var hasError = false;
			$('#contactForm-widget1 .requiredField').each(function () {
				if (jQuery.trim($(this).val()) === '') {
					var labelText = $(this).prev('label').text();
					$(this).addClass('fielderror');
					$('#contactForm-widget1 span').html('<strong>*Please fill out all fields.</strong>');
					hasError = true;
				} else if ($(this).hasClass('email')) {
					var emailReg = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
					if (!emailReg.test(jQuery.trim($(this).val()))) {
						var labelText = $(this).prev('label').text();
						$(this).addClass('fielderror');
						$('#contactForm-widget1 span').html('<strong>Is incorrect your email address</strong>');
						hasError = true;
					}
				}
			});
			if (!hasError) {
				$('#contactForm-widget1').slideDown('normal', function () {
					$("#contactForm-widget1 #sendMessage").addClass('load-color');
					$("#contactForm-widget1 #sendMessage").attr("disabled", "disabled").val('Sending message. Please wait...');
					$('#contactForm-widget1 span').html('<i class="icon-spinner icon-spin"></i>');
				});
				var formInput = $(this).serialize();
				$.post($(this).attr('action'), formInput, function (data) {
					$('#contactForm-widget1').slideUp("normal", function () {
						$(this).before('<div class="notification-box notification-box-success"><p><i class="icon-ok"></i>Thanks!</strong> Your email was successfully sent. We check Our email all the time, so we should be in touch soon.</p></div>');
					});
				});
			}
